...
### Costruzione rapida con helper (consigliata)

Usa gli helper a campi piatti (`cedentePrestatore`, `cessionario`) insieme ad
`addLinea` per evitare oggetti annidati e calcoli IVA manuali. Il builder calcola
`DatiRiepilogo` e `ImportoTotaleDocumento` dalle linee, e `toResult()` restituisce
oggetto, XML, esito validazione e nome file SDI in un'unica chiamata.
...
const { xml, filename, validazione } = FatturaBuilder.create()
  ...
  // numeroLinea, prezzoTotale, datiRiepilogo e importoTotaleDocumento sono calcolati automaticamente
  .addLinea({ descrizione: 'Servizio di consulenza', quantita: 10, unitaMisura: 'ore', prezzoUnitario: 100, aliquotaIVA: 22 })
  .toResult();
...
::callout
Gli helper convertono i valori a stringa internamente: i campi che arrivano dal
database (P.IVA, CAP, telefono parsati come `number`) non fanno più crashare il
serializer — non serve convertirli a mano.
::

L'API fluent completa qui sotto resta supportata per i casi avanzati
(`DatiRiepilogo` espliciti, fatture lotto, ecc.).
...
// Metodo 3: Direttamente dal builder (senza chiamare build() prima)
