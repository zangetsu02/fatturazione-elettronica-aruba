...
### Quick build with helpers (recommended)

Use the flat-input helpers (`cedentePrestatore`, `cessionario`) together with
`addLinea` to skip nested objects and manual VAT math. The builder computes
`DatiRiepilogo` and `ImportoTotaleDocumento` from the lines, and `toResult()`
returns the object, the XML, the validation result and the SDI filename in one call.
...
const { xml, filename, validazione } = FatturaBuilder.create()
  .setTrasmissioneB2B({
    idPaese: 'IT',
  ...
  .setCessionarioCommittente(cessionario({
    ...
    indirizzo: 'Via Milano 10', cap: '20100', comune: 'Milano', provincia: 'MI',
  }))
  .setDatiGenerali({
    datiGeneraliDocumento: { tipoDocumento: 'TD01', divisa: 'EUR', data: '2024-01-15', numero: '1' },
  })
  // numeroLinea, prezzoTotale, datiRiepilogo and importoTotaleDocumento are computed automatically
  .addLinea({ descrizione: 'Consulting service', quantita: 10, unitaMisura: 'hours', prezzoUnitario: 100, aliquotaIVA: 22 })
  .toResult();

// validazione.valid === true
// filename === 'IT01234567890_00001.xml'
```

::callout
The helpers coerce values to strings internally, so fields coming from a database
(VAT numbers, ZIP codes, phone numbers parsed as `number`) no longer crash the
serializer — you don't need to convert them manually.
::

The full fluent API below remains supported for advanced cases (explicit
`DatiRiepilogo`, batch invoices, etc.).
...
// Method 3: Directly from the builder (no need to call build() first)
const result = FatturaBuilder.create()/* ... */.validate({ strict: true });
